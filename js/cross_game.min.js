...
var v=i(_.concat([0]))
y.push(v),v.length>h&&(h=v.length)}for(var g=35,k=null,w=50,b=50,x=20,I=20*h,m=25,C=m,j=I,S=a*g+j+50,z=1e3,A=d3.select("#canvas").append("svg").attr("width",S).attr("height",z),f=0;a>f;f++)for(var u=0;a>u;u++){var B=u*g+j+x,R=f*g+I+x
...
var C=d3.select(this).attr("id"),j="cross_"+C,S=d3.select("path[id='"+j+"']")
if(1==S.size())S.remove()
else if(0==S.size()){var $=parseInt(d3.select(this).attr("x"))+g/2,z=parseInt(d3.select(this).attr("y"))+g/2
A.append("path").attr("d",d3.svg.symbol().size(350).type(function(t){return"cross"})).attr("id",j).attr("class","cross").attr("transform","translate("+$+","+z+") rotate(45)").attr("fill","red").on("click",function(){d3.select(this).remove()})}}})
